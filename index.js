...
function get20s(lifeData){
  let newArr = [];
  for (let i=0; i<=lifeData.length-1; i++){
    if ((lifeData[i].years).startsWith('19') && (lifeData[i].years).startsWith('19', 7)){
      newArr.push(lifeData[i].name)
    }
  }
  return newArr;
}
...
function addArtist(arr, id, name, years, genre, nationality, bio){
  ...
  // console.log(addArtist(artists, 20, 'Juan', '1997-Present', 'Web Design', 'Mexican', 'Swiggity Swag Whats In The Bag' ))
